...
# VS Code Mock Server 插件 - 产品需求文档（PRD）

## 1. 概述（Overview）
VS Code Mock Server 插件旨在为开发者提供一种在本地项目中快速、可视化、可配置地构建 Mock API 服务的方式。  
...
### 场景 1：前端调试未完成的接口
用户在 VS Code 内查看 Mock API → 启动 Mock Server → 前端请求指向 `localhost:9527` → 快速完成调试。
...
添加一个独立图标“Mock Server”，点击后进入插件主界面。
